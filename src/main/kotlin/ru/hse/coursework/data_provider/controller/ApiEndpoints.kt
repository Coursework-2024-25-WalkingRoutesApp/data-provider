package ru.hse.coursework.data_provider.controller

const val USER_ID_HEADER = "X-User-Id"

const val ROUTE_BASE_PATH_URL = "/api/route-provider/routes"
const val ADD_ROUTE_URL = "/user/add"
const val GET_DRAFTS_URL = "/user/drafts"
const val GET_PUBLISHED_URL = "/user/published"
const val DELETE_ROUTE_URL = "/user/delete"
const val GET_ROUTE_PAGE_URL = "/user/page"
const val GET_ROUTE_BY_SEARCH_VALUE_URL = "/user/search"
const val GET_ROUTES_URL = "/user/all"

const val FAVORITE_BASE_PATH_URL = "/api/route-provider/favorites"
const val GET_FAVOURITES_URL = "/user/all"
const val ADD_FAVOURITE_URL = "/user/add"
const val DELETE_FAVOURITE_URL = "/user/delete"

const val SESSION_BASE_PATH_URL = "/api/route-provider/sessions"
const val GET_FINISHED_URL = "/user/finished"
const val GET_UNFINISHED_URL = "/user/unfinished"
const val ADD_SESSION_URL = "/user/add"
const val GET_SESSION_URL = "/user"

const val REVIEW_BASE_PATH_URL = "/api/route-provider/reviews"
const val GET_REVIEWS_URL = "/user/all"
const val ADD_REVIEW_URL = "/user/add"

const val USER_BASE_PATH_URL = "/api/route-provider/security"
const val REGISTER_URL = "/register"
const val LOGIN_URL = "/login"
const val GET_USER_INFO_URL = "/info"
const val UPDATE_USERNAME_URL = "/update-username"
const val GET_USER_BY_EMAIL_URL = "/by-email"

const val METRICS_BASE_PATH_URL = "/api/route-provider/metrics/admin"
const val GET_REGISTERED_USERS_COUNT_URL = "/registered/count"
const val GET_TOTAL_LIKES_COUNT_URL = "/likes/total"
const val GET_FINISHED_ROUTES_PER_USER_URL = "/finished/per-user"
const val GET_REVIEWS_COUNT_PER_ROUTE_URL = "/reviews/per-route"
const val GET_PUBLISHED_ROUTES_COUNT_PER_USER_URL = "/published/per-user"