...
import ru.hse.coursework.data_provider.api.RouteControllerApi
...
class RouteController(
    ...
    override fun addRoute(@RequestHeader(USER_ID_HEADER) userId: UUID, @RequestBody routeDto: RouteDto): ResponseEntity<String> =
        routeService.createOrUpdateRoute(routeDto, userId)

    @GetMapping(GET_DRAFTS_URL)
    override fun getDrafts(@RequestHeader(USER_ID_HEADER) userId: UUID, @RequestParam latitude: Double, @RequestParam longitude: Double): List<RouteCartDto> =
        routeService.getUserRouteDrafts(userId, UserCoordinateDto(latitude, longitude))

    @GetMapping(GET_PUBLISHED_URL)
    override fun getPublished(@RequestHeader(USER_ID_HEADER) userId: UUID, @RequestParam latitude: Double, @RequestParam longitude: Double): List<RouteCartDto> =
        routeService.getUserPublishedRoutes(userId, UserCoordinateDto(latitude, longitude))

    @DeleteMapping(DELETE_ROUTE_URL)
    override fun deleteRoute(@RequestHeader(USER_ID_HEADER) userId: UUID, @RequestParam routeId: UUID): ResponseEntity<String> =
        routeService.deleteRoute(routeId, userId)

    @GetMapping(GET_ROUTE_PAGE_URL)
    override fun getRoutePage(@RequestHeader(USER_ID_HEADER) userId: UUID, @RequestParam routeId: UUID): RoutePageDto =
        routeService.getRoutePage(routeId, userId)

    @GetMapping(GET_ROUTE_BY_SEARCH_VALUE_URL)
    override fun getRouteByName(
